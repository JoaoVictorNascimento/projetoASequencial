...
header = 'exp,execution,benchmark,size_of_data,schedule,chunk_size,num_threads,version,num_threads,NI,NJ,NK,ORIG,OMP'

def parse(data):

   print ('parsing...')

   result = re.findall('exp = (.*?), execution = (.*?), benchmark = (.*?), size_of_data = (.*?), schedule = (.*?), chunk_size = (.*?), num_threads = (.*?),\n.*?\n*version = (.*?), num_threads = (.*?), NI = (.*?), NJ = (.*?), NK = (.*?), ORIG = (.*?), OMP = (.*?), \n', data, re.DOTALL)
