...
list.of.packages <- c("plyr", "ggplot2", "scales", "pdftools")
...
myLoadAndPrepareData<-function(p_experiment_name, p_benchmark_name, p_csv_file_name)
{
  csv_file_name <- paste0(p_csv_file_name)
...
myFilterAndPlot<-function(p_experiment_name, p_benchmark_name, p_num_threads, p_chunk_size)
{
  ...
  plot.subtitle <- paste0(p_experiment_name)
  
  graph_file_name <- paste0("graph-omp-", p_experiment_name,"-benchmark-",p_benchmark_name,"-num_threads-",p_num_threads,"-chunk_size-",p_chunk_size,".pdf")
  
  csv_file_name <- paste0("data-omp-", p_experiment_name,"-benchmark-",p_benchmark_name,"-num_threads-",p_num_threads,"-chunk_size-",p_chunk_size,".csv")
...
work_dir_name_base <- paste0("graph")
experiment_name <- paste0("all-data-sizes-and-chunk-sizes-evaluation-openmp-for")
...
num_threads_configs <- c(16,8,4,2,1)
chunk_size_configs <- c(16,32,64,128,256)

# Experimento ragserver.
benchmark_name <- paste0("gemm")
csv_data_file = "gemm-joined-results-processed-final.csv"
                
# Carrega os dados.
myLoadAndPrepareData(experiment_name, benchmark_name, csv_data_file)
...
for (nt in num_threads_configs) {
  for (cs in chunk_size_configs) {
    ...
    myFilterAndPlot(experiment_name, benchmark_name, p_num_threads = nt, p_chunk_size = cs)
  }  
}
